...
MATRIX = ROOT / "peer_review" / "coverage" / "PEER_REVIEW_PAPER_COVERAGE_MATRIX.csv"
MASTER_PATHS = [
...
TZP_ID = ROOT / "tzp_id"
OUT_DIR = ROOT / "peer_review" / "coverage" / "core_targeted_wolfram"
...
def build_queue(master_rows: dict[str, dict[str, str]]) -> tuple[list[dict[str, object]], dict[str, set[str]]]:
    ...
    with MATRIX.open("r", encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            if row["paper_kind"] != "core" or row["coverage_status"] != "partial_certificate":
                continue
...
def main() -> None:
    ...
    queue_path = OUT_DIR / "core_targeted_wolfram_queue.json"
    results_path = OUT_DIR / "core_targeted_wolfram_results.json"
    runner_path = OUT_DIR / "run_core_targeted_wolfram.wls"
    ...
    csv_path = OUT_DIR / "CORE_TARGETED_WOLFRAM_CERTIFICATE.csv"
    ...
    md = []
    md.append("# Core Paper Targeted Wolfram Certificate\n")
    md.append(f"Generated: {datetime.now(timezone.utc).isoformat(timespec='seconds')}\n")
    md.append("## Scope\n")
    md.append("- Target: blank or not-run Wolfram statuses cited by the 10 numbered core papers.")
    ...
    md.append("")
    md.append("## Result\n")
    md.append("| ID | Core papers | Blocks | Passes | Folded status |")
    md.append("| --- | --- | ---: | ---: | --- |")
    ...
    (OUT_DIR / "CORE_TARGETED_WOLFRAM_CERTIFICATE.md").write_text("\n".join(md), encoding="utf-8")

    print(f"target_ids={len(by_id_papers)} blocks={len(results)} updated={len(updates)}")
    print(f"wrote {OUT_DIR / 'CORE_TARGETED_WOLFRAM_CERTIFICATE.md'}")
